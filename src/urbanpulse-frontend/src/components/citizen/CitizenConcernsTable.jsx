 ...
 * Brief Description: Displays citizen-reported concerns and their current statuses.
 */
  ...
  return (
    <section className="mb-10">
      <div className="mb-4 flex items-center justify-between">
        ...
        {safeConcerns.length > 0 && !loading && (
          <span className="rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-semibold text-slate-600">
            {safeConcerns.length}{" "}
            {safeConcerns.length === 1 ? "report" : "reports"}
          </span>
        )}
      </div>
        ...
        {error ? (
          <div className="flex flex-col items-center gap-3 px-6 py-10 text-center">
            ...
            {onRetry && (
              <button
                type="button"
                onClick={onRetry}
                className="flex items-center gap-2 rounded-lg border border-slate-200 px-3.5 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors"
              >
          ...
          <div className="flex flex-col items-center gap-2 px-6 py-12 text-center">
            ...
            <p className="text-xs text-slate-400">
              Concerns you submit will show up here for you to track and manage.
            </p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full min-w-[620px] text-left text-sm">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50/60 text-xs uppercase tracking-wide text-slate-500">
                    <th className="pl-6 pr-5 py-3 font-medium text-left">
                      Category
                    </th>
                    ...
                    <th
                      colSpan={2}
                      className="pl-5 pr-6 py-3 font-medium text-right"
                    >
                      Actions
                    </th>
                  </tr>
                        ...
                        <tr className="transition hover:bg-slate-50/60">
                          ...
                          <td className="px-5 py-3.5">
                            <span
                              className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium capitalize ${
                                PRIORITY_STYLES[
                                  String(concern.priority).toLowerCase()
                                ] ||
                                "bg-slate-100 text-slate-600 ring-1 ring-inset ring-slate-200"
                              }`}
                            >
                          ...
                          <td className="pl-5 pr-2 py-3.5 text-right">
                            <button
                              ...
                              className="flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-medium text-emerald-700 hover:bg-emerald-50 ml-auto transition-colors"
                            >
                              View
                              <ChevronRight
                          ...
                          {/* Delete Action */}
                          <td className="pl-1 pr-6 py-3 text-right w-10">
                            {canDelete && onSelectDelete ? (
                              <button
                                type="button"
                                onClick={() => onSelectDelete(concern)}
                                aria-label="Delete concern"
                                className="rounded-lg p-1.5 text-slate-400 hover:bg-red-50 hover:text-red-600 inline-block transition-colors"
                              >
                    ...
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          type="button"
                          onClick={() =>
                            setExpandedConcernId(isExpanded ? null : concernId)
                          }
