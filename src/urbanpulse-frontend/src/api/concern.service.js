 ...
 * Brief Description: Handles API requests for reporting and managing citizen concerns.
 */
...
export const concernService = {
  ...
  updateConcernStatus: async (concernId, status, remarks = null) => {
    const payload = {
      status: String(status).toLowerCase(),
      ...(remarks ? { remarks } : {}),
    };
    const response = await api.patch(`/concerns/${concernId}/status`, payload);
    return response.data;
  },
