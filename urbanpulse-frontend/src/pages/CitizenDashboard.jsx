...
function getPriorityStyle(priority) {
  ...
  return PRIORITY_STYLES[key] || 'bg-slate-100 text-slate-600 ring-1 ring-inset ring-slate-200';
}

function getSuggestionStatusStyle(status) {
  const key = String(status || '').toLowerCase();
  return SUGGESTION_STATUS_STYLES[key] || 'bg-slate-100 text-slate-600 ring-1 ring-inset ring-slate-200';
}
...
function StatusBadge({ styleClass, label, dot }) {
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium ${styleClass}`}>
        ...
        >
          ...
          >
            ...
            <p className="mt-1.5 text-sm text-slate-500">
              You're about to delete <span className="font-medium text-slate-700">"{concern.category || concern.title || 'Concern'}"</span>
              {concern.location && <> at <span className="font-medium text-slate-700">{concern.location}</span></>}. This can't be undone.
            </p>
...
export default function CitizenDashboard() {
  ...
  const fetchDashboard = useCallback(async () => {
    ...
    try {
      ...
      setDashboardError(extractErrorMessage(err, 'Unable to load dashboard data.'));
    } finally {
  ...
  const fetchConcerns = useCallback(async () => {
    ...
    try {
      ...
      setConcernsError(extractErrorMessage(err, 'Unable to load your concerns.'));
    } finally {
  ...
  const fetchSuggestions = useCallback(async () => {
    ...
    try {
      ...
      setSuggestionsError(extractErrorMessage(err, 'Unable to load your suggestions.'));
    } finally {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setDashboardData((prev) =>
        prev
          ? {
              ...
              pending_concerns: DELETABLE_STATUSES.includes(String(concernToDelete.status).toLowerCase())
      ...
      showToast('error', extractErrorMessage(err, 'Could not delete this concern. Please try again.'));
    } finally {
  ...
  const handleSubmitSuggestion = async (e) => {
    ...
    try {
      ...
      if (err?.response?.status === 422) {
        ...
        showToast('error', extractErrorMessage(err, 'Could not submit collection point. Please try again.'));
      }
    } finally {
      setSubmitting(false);
    }
  ...
  const userName = userProfile?.full_name || userProfile?.name || 'Sneha Kesharwani';
    ...
    <div className="flex shrink-0 items-center gap-3 sm:gap-4">
      ...
      <main className="mx-auto max-w-[1400px] px-4 py-8 sm:px-6 lg:py-10">
        <div className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">Citizen Dashboard</h1>
                  ...
                  <table className="w-full min-w-[720px] text-left text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {concerns.map((concern) => {
                        ...
                        const canDelete = DELETABLE_STATUSES.includes(String(concern.status).toLowerCase());
                            ...
                            <tr className="transition hover:bg-slate-50/60">
                              <td className="px-5 py-3.5">
                                <div className="flex items-center gap-2.5">
                                  ...
                                  <span className="font-medium text-slate-800">{concern.category || 'General'}</span>
                                </div>
                              </td>
                              <td className="max-w-[220px] truncate px-5 py-3.5 text-slate-600">{concern.location || '—'}</td>
                              <td className="px-5 py-3.5 text-slate-600">{formatDate(concern.created_at)}</td>
                              <td className="px-5 py-3.5">
                                <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium capitalize ${getPriorityStyle(concern.priority)}`}>
                                  {concern.priority || 'N/A'}
                                </span>
                              </td>
                              <td className="px-5 py-3.5">
                                <StatusBadge styleClass={statusStyle.badge} label={statusStyle.label} dot={statusStyle.dot} />
                              </td>
                              <td className="px-5 py-3.5">
                      ...
                      <div key={concernId} className="p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex items-start gap-2.5">
                            ...
                            <div>
                              <p className="font-medium text-slate-800">{concern.category || 'General'}</p>
                              <p className="text-xs text-slate-500">{concern.location || '—'}</p>
                            </div>
                          </div>
                          {canDelete && (
                            <button
        ...
        <section>
          <div className="mb-4 flex items-center justify-between">
            ...
            {typeof dashboardData?.total_suggestions === 'number' && (
              <span className="text-xs font-medium text-slate-400">{dashboardData.total_suggestions} total</span>
            )}
          </div>
                  ...
                  return (
                    <div key={sugId} className="p-5">
                      <div className="flex flex-wrap items-start justify-between gap-2">
                        <p className="font-semibold text-slate-800">{suggestion.title || `Point #${sugId}`}</p>
                        <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium capitalize ${getSuggestionStatusStyle(suggestion.status)}`}>
                          {toTitleCase(suggestion.status || 'pending')}
                        </span>
                      </div>
                      <p className="mt-1.5 text-sm text-slate-600">{suggestion.description || `Lat: ${suggestion.latitude}, Lng: ${suggestion.longitude}`}</p>
                      <p className="mt-2 text-xs text-slate-400">Submitted {formatDate(suggestion.created_at)}</p>
        ...
        {drawerOpen && (
          <>
            ...
            >
              ...
              <form onSubmit={handleSubmitSuggestion} className="flex flex-1 flex-col overflow-y-auto px-6 py-5">
